...
def primary(oc):                  # oc - opcode
    ...
    if int(oc) == 0:
        ...
        Register.put_to_reg('01100100',oc[8:16])

    elif int(oc[:8]) == 11:
        # print('<ureg> = POP PCSTK')
        Register.put_to_reg(oc[8:16],Register.get_from_reg('01100100'))
        # deleting value in PCSTK not done
        Register.put_to_reg('01100100',"XXXX")
        
    elif int(oc[:6]) == 11:
        #print('immediate data(16-bits) -> ureg')
        Register.put_to_reg(oc[8:16],oc[16:])
        # print(oc[8:16],get_from_reg(oc[8:16]))

    elif int(oc[1:6]) == 1:
        # print('IF condition ureg1 = ureg2')
        # ureg1 as 'dest' and ureg2 as 'source'
        Register.put_to_reg(oc[8:16],Register.get_from_reg(oc[16:23]))
