...
def multi(op):
    ...
    if(op[0:2]=="00"):
        ...
        if(op[0:2]=="01"):  # rx*ry 
            ...
            val=0
            if(op[5]=="1"):
                mrf1=fbin_to_decimal(mrf,int(op[3])|int(op[4]))+fmul(rx,ry,op[3],op[4])
                if(mrf1<(-1)):
                ...
                mrf1 = int(mrf,2)+int(unsigned_mul(rx,ry),2)
                mrf1 = format(mrf1,'040b')
            else: #signed multiplication
                mul = signed_mul(rx,ry,op[4],op[3])
                mrf1 = int(mrf,2)+int(mul,2)
                mrf1 = format(mrf1,'040b')
        else:                   #rn = mr-rx*ry
            val=0
            if(op[5]=="1"):
                ...
                mrf1 = int(mrf,2)-int(signed_mul(rx,ry,op[4],op[3]),2)
                mrf1 = format(mrf1,'040b')
            if(mrf1[0]=="-"):
